...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from Cliente where login_cliente ='${login}' and senha ='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  telefone = req.body.telefone;
    ...
    if (nome == undefined || endereco == undefined || telefone == undefined || login == undefined || senha == undefined) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${nome} / ${endereco} / ${telefone} / ${login} / ${senha}`);
    }
    return banco.sql.query(`select count(*) as contagem from Cliente where login_cliente = '${login}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into Cliente (nome,endereco,telefone,login_cliente,senha) values ('${nome}', '${endereco}', '${telefone}', '${login}', '${senha}')`).then(function() {
